...
void loop() {
  ...
  FRONT = round((FilterFront.run((duration/2) / 29.1)/5))*10;  
  ...
  RIGHT = round((FilterRight.run((duration/2) / 29.1)/5))*10;
  ...
  LEFT = round((FilterLeft.run((duration/2) / 29.1)/5))*10;
  
  const float sensorRange = 100;
  ...
  if(RIGHT < sensorRange) {
    ...
    rangeClampedRight = 100;
  }

  float vX = rangeClampedFront;
  float vY = rangeClampedLeft;
  float vYmed = rangeClampedLeft - rangeClampedRight;
